				...
				<tbody>
					<c:forEach items="${board.list}" var="b">
						<tr>
							...
							<td><a
								href="${pageContext.request.contextPath}/admin/board/detail/${b.adminBoardIdx}">${b.title}</a>
							</td>
			...
			<div class="row justify-content-md-center mt-4 bottom">
				<div class="btn-toolbar" role="toolbar" style="display: flex; align-content: center; justify-content: center;">
					<div class="btn-group mr-2" role="group">
						...
						<c:forEach begin="${board.pagination.startNum}"
							end="${board.pagination.endNum}" var="pnum">
							<a href="announcement?p=${pnum}"
								class="btn ${board.pageNum eq pnum ? 'btn-dark': 'btn-white'}">${pnum}</a>
						</c:forEach>
						<c:if test="${board.pagination.nextNum>0}">
							<a href="announcement?p=${board.pagination.nextNum}"
